...
1. Login into the Azure subscription that you'll be deploying into.  To make sure your resource group names will not conflict with others within the same subscrioptions, you will be assigned a number during the workshop. Please replace 10 with your assigned number.
   ...
   ```bash
   rgNumber=10
   hubs="rg-enterprise-networking-hubs-"$rgNumber
   spokes="rg-enterprise-networking-spokes-"$rgNumber
   aks="rg-bu0001a0008-"$rgNumber
...
1. Create the networking hubs resource group. 
   ...
   az group create -n $hubs -l centralus
   ```
...
1. Create the networking spokes resource group.
   ...
   ```bash
   # [This takes less than one minute to run.]
   az group create -n $spokes -l centralus
   ```
...
1. Create the regional network hub.
   ...
   az deployment group create -g $hubs -f networking/hub-default.json -p location=eastus2
   ```
